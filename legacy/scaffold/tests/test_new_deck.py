...
class TestGalleryMode:
    ...
    def test_structure_only_emits_slot_hint_no_text_overrides(self, tmp_path):
        """When no sibling demo deck is found, the gallery emits a slot-
        name hint comment and NO ``::slot::`` text overrides. This avoids
        the overflow bug: inventing placeholder text breaks layouts whose
        boxes are sized for the template's actual content."""
        theme = _make_valid_theme(tmp_path / "slidev-theme-test")
        ...
        # Hint comment lists the available slots so the user knows.
        assert "Slots available" in slides
        for s in ("title", "body-12", "body-19"):
            assert s in slides
        # But NO ``::slot::`` blocks on their own lines (those would
        # produce overflow because placeholder text would overflow the
        # layout's fixed-width boxes).
        bare_blocks = [line for line in slides.splitlines() if line.startswith("::") and line.endswith("::")]
        assert bare_blocks == [], (
            f"Structure-only mode should emit no ``::slot::`` blocks, "
            f"got: {bare_blocks}"
        )

    def test_picture_slots_listed_in_hint(self, tmp_path):
        """Picture slots are listed separately in the hint comment so the
        user knows the default image will show + how to override it."""
        theme = _make_valid_theme(tmp_path / "slidev-theme-test")
        ...
        # No active ``::picture-22::`` override either (would suppress
        # the layout's default image).
        assert not any(line == "::picture-22::" for line in slides.splitlines())


class TestDemoDeckCopy:
    """When the theme has a sibling demo deck (typical for themes
    generated by import-template), the gallery copies the demo deck
    verbatim and just patches the frontmatter. The demo deck's content
    was authored for each layout's fixed-width box → it actually fits,
    unlike anything we could invent at scaffold time."""

    def _make_theme_with_demo(self, base: Path, theme_name: str = "slidev-theme-test") -> tuple[Path, Path]:
        """Create a theme dir AND a sibling demo deck at <base>/deck/slides.md."""
        theme = _make_valid_theme(base / "slidev-theme-test", name=theme_name)
        _add_layout(theme, "slide1", ["title", "body-12"])
        _add_layout(theme, "slide2", ["title"])

        demo_dir = base / "deck"
        demo_dir.mkdir(parents=True, exist_ok=True)
        demo_slides = demo_dir / "slides.md"
        demo_slides.write_text(
            "---\n"
            f"theme: {theme_name}\n"
            "layout: slide1\n"
            "---\n"
        ...
        return theme, demo_slides

    def test_copies_demo_deck_content_verbatim(self, tmp_path):
        theme, demo = self._make_theme_with_demo(tmp_path)
        deck = tmp_path / "my-deck"
        scaffold_deck(deck, theme, "my-deck", install=False)
        slides = (deck / "slides.md").read_text()

        # Demo deck's example content lands in the new deck — unchanged.
        assert "BIG TITLE" in slides
        assert "Example" in slides
        assert "Second Slide" in slides

    def test_patches_theme_name_in_frontmatter(self, tmp_path):
        # Demo deck was authored for one theme; the new deck consumes a
        # *renamed* theme. The first frontmatter's theme: line must be
        # rewritten to point at the new name.
        theme = _make_valid_theme(tmp_path / "slidev-theme-real", name="slidev-theme-real")
        _add_layout(theme, "slide1", ["title"])

        # Demo deck stamped with a stale theme name.
        demo_dir = tmp_path / "deck"
        demo_dir.mkdir()
        (demo_dir / "slides.md").write_text(
            "---\ntheme: slidev-theme-old\nlayout: slide1\n---\n\n::title::\nHi\n",
            encoding="utf-8",
        )
        ...
        assert "theme: slidev-theme-real" in slides
        assert "slidev-theme-old" not in slides

    def test_falls_back_to_structure_only_when_no_demo(self, tmp_path):
        """No sibling deck/ folder → use structure-only fallback (no
        text overrides, just slot-name hint comments)."""
        # Note: _make_valid_theme builds a theme but no sibling demo deck.
        theme = _make_valid_theme(tmp_path / "slidev-theme-test")
        _add_layout(theme, "slide1", ["title"])
        deck = tmp_path / "my-deck"
        scaffold_deck(deck, theme, "my-deck", install=False)
        ...
        # Structure-only fallback signature: the gallery-mode HTML comment
        # at the top + a "Slots available" hint.
        assert "Slots available" in slides
