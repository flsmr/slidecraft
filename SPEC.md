...
Companion to `ARCHITECTURE.md` (the *what/why*)
...
| **D34** | **Triage by parking (the Storyteller's third move, beside merge).** Every nugget **always gets a slide** — none are left unplaced. When the budget is full and a nugget arrives, the Storyteller may (a) associate to a fitting slide, (b) **merge** two condensable slides, or (c) **park** a lower-priority / off-storyline slide to free an active slot, then create the newcomer. *(Post-D41: associate/merge/park/create are **decision types in the plan**, executed by the orchestrator; parked-slide knowledge reaches the Storyteller via `plan-brief`'s deck-state injection, not by reading files.)* **Park** = the slide's `src:` include is moved into the **"Backup Slides" appendix block at the end of `slides.md`** (rendered, not hidden — revised by D46; was a commented block); the slide **file and association are preserved** (composed prose kept, D31-style), and a bodyless parked slide gets a deterministic digest body (D46). The Backup block is part of the injected deck state, so a later story shift can un-park it (which recomposes it — D46). `km` verbs: `park-slide`, `unpark-slide` (needs a free active slot), `create-slide --parked` (create directly parked when the newcomer itself is lowest-priority and the deck is full). **Budget counts only active slides** — a parked slide carries `state:"parked"` (km's source of truth, D46) and is excluded by it; the inline Backup divider is not a slide file, so it is not counted either (revises D14: structural counts, parked does not). Consequence: **backlog (D1) is now vestigial** — set-aside knowledge is *visible Backup slides* (D46), not an abstract pool; D1 is demoted to correction-only. |
...
| **D46** | **Parked slides render as a "Backup Slides" appendix (evolves D34, 2026-07-21).** A parked slide is no longer hidden — it is **surfaced at the end of the deck for human review**, so the human can see what the storyteller set aside and decide keep-or-hide in the final polish. Changes to D34's `park`: **(a)** `park-slide` moves the slide's `src:` include into a real, **uncommented Backup block at the end of `slides.md`** (was: a commented block), appending in park order. **(b)** `km` **auto-manages a "Backup Slides" section divider** at the block boundary — an **inline markdown slide written straight into `slides.md`** (not a slide file, so no budget/validate path counts it), inserted when the block goes empty→non-empty and removed when it empties; the storyteller never plans it, so the plan schema is unchanged. **(c) Distilled body, no LLM:** when a slide lands in the Backup block **with no body yet** (a directly-parked create, or a `pending` slide), `km` deterministically writes a digest-dump body from each associated nugget's **`information`** field (title heading + bullets), so the human roughly knows the nugget's content; a slide that already carries a **composed** body keeps it (never overwritten). Like the D39 notes fill, this is **mechanical assembly of already-distilled miner output, not composed prose** — a deliberate, narrow carve-out to D31, not a violation of it. **(d)** `unpark-slide` **resets the slide to `pending` and discards the digest body**; the orchestrator then composes it for real — so **unpark joins create + merge as a compose trigger**. **(e)** Parked slides carry **`state:"parked"`** — km's source of truth (parking shelves the prior lifecycle state in `state_before_park` and restores it on unpark). The budget gate and `validate` exclude them by that state; `order()` filters them out of the active order by it too (it can no longer rely on the old comment boundary, since the Backup block now renders). The inline divider is plain `slides.md` markdown, so nothing counts it. **No per-slide frontmatter flag and no theme/footer change in v1** (the human may comment out the appendix or drop the total-slides counter in final polish). `validate` **lists** backup slides (`backup_slides`: slide + title + reason) but does **not** flag them as unfinished. |

**Decision added 2026-07-22 (slide design variants):**

| # | Decision |
|---|---|
| **D47** | **Slide design variants + in-place cycling.** A slide may hold several coexisting renderings. The **active** one is the postfix-less `slides/<sid>.md` that `slides.md` includes; alternatives are `slides/<sid>_vN.md`. **The filesystem is the only state** — no manifest, no index file, no per-variant `.json` (a slide's variants **share one state file**), no frontmatter flag. **Selection = rename:** `km cycle-variant --slide ID --dir up\|down` ring-rotates which file is canonical (via a `.cycletmp` scratch, one temp, cascade), so `slides.md` is **never edited**; whatever is active when the user leaves the slide is the pick. `km get-variants --slide ID` (pure read) enumerates a slide's renderings by globbing. **Live in the browser:** a ~30-line Vite `configureServer` endpoint + a `setup/shortcuts.ts` remap **↑/↓** to POST a cycle; the swapped include triggers Slidev's reload **in place** (nav index persists via the URL). **Enumeration filter:** a canonical slide file is any `slides/*.md` **not** matching `_v\d+\.md$`; variant files are invisible to `order` / `validate` / budget (they are renderings of an existing slide, not new slides) — the `.json`-keyed helpers need no change. **Merge preserves provenance (evolves D31):** `merge-slides` no longer deletes the predecessors — it renames each predecessor (and its existing variants) into the new slide's `_vN` variants, and the fresh union compose becomes the **active** variant (so merging two plain slides yields the merged slide + two predecessor variants). **Scope: mechanics only** — the differentiated *creation* of variants (image-gen vs. diagram vs. text "lanes") is deferred; until then variants arise **only** from merge. **Load-bearing assumption, spiked first:** Slidev reloads on an included-file **rename** (unlink+create) and returns to the same slide, and `shortcuts.ts` can read the current slide's source path. See **ADR-0004** and `docs/superpowers/specs/2026-07-22-slide-variant-cycling-design.md`. |
...
| `create-slide --title T --nuggets a,b --after ID\|end [--parked]` | Stamped skeleton `.md` + state `.json` (`draft`) + association entry + inserts into `slides.md`. **Atomic budget gate:** exits non-zero `{"error":"budget_full"}` when **active** slides reach `max_slides` (structural counts, parked does not — D34). `--parked` creates directly into the **Backup appendix block** (no active-slot use; auto-inserts the "Backup Slides" divider if absent) and, since the slide is bodyless, writes a **deterministic digest body** from the associated nuggets' `information` (D46). Never writes composed prose. | orchestrator (executing the plan, D41) |
| `merge-slides --slides a,b --title T` | New stamped slide carrying the **union** of inputs' nuggets; **renames each original (and its existing variants) into the new slide's `_vN` variants for provenance (D47 — was: deletes them)**; fixes `slides.md` (frees a slot). Never writes body (D31 — Composer runs after, as the active variant). | orchestrator (executing the plan, D41) |
| `park-slide --slide ID` / `unpark-slide --slide ID` | **park:** move the `src:` include into the rendered **"Backup Slides" appendix block** at the end of `slides.md` (auto-inserts the divider when the block was empty); file + association untouched (D34); if the slide has **no body yet**, write a deterministic digest body from its nuggets' `information`, but **never overwrite a composed body** (D46). **unpark:** needs a free active slot; move the include back to the active flow, **reset the slide to `pending` and discard the digest body** (the orchestrator recomposes it — D46), and remove the divider if the block is now empty. | orchestrator (executing the plan, D41) |
...
| `plan-brief` | **[D40/D42, TO BUILD]** Renders the self-contained storyteller brief: role template + deck constraints + inlined storytelling craft + **all nugget digests** (`title`/`information`/`figure_type`/`description` — never `raw_text`/`visible_text`) + current deck state (incl. the Backup appendix block) on a re-run. | orchestrator |
...
| `cycle-variant --slide ID --dir up\|down` | **[D47, TO BUILD]** Ring-rotates which of a slide's coexisting renderings is the canonical `slides/<sid>.md`, among it + its `_vN` siblings, by rename (one `.cycletmp` scratch; cascade so each destination is vacated first). `<2` renderings → no-op. `slides.md` and the shared state `.json` are **untouched** — the active file IS the selection. Clears any stale `.cycletmp` on entry. | Vite endpoint / orchestrator |
| `get-variants --slide ID` | **[D47, TO BUILD]** Pure read: globs `slides/` and returns `{count, files[]}` for a slide's renderings (canonical + `_vN`). Writes nothing. The dev-server endpoint asks it on slide entry to decide whether to intercept ↑/↓. | dev-server endpoint |
| `validate` | md↔state pairing, associations resolve, stamp uniqueness, `slides.md` complete/duplicate-free, budget respected (Backup block excluded by position). **Variant files (`_vN`) are ignored** — alternative renderings of an existing slide, not slides (D47). **Backup slides are valid, not unfinished** — reported separately (`Backup Slides (parked): N` + titles), never flagged like a stray `pending` (D46). | lead |
...
/draft-deck (lead / orchestrator — every role runs assemble → invoke → persist, D40)
 ...
 4 EXECUTE +   the ORCHESTRATOR walks the plan: km create-slide/merge-slides/park-slide/
   COMPOSE     unpark-slide; after every create, merge, AND unpark: km compose-brief --slide
               (routed raw fields, D42) → invoke (OWUI) → km write-slide (semantic JSON →
               physical Slidev, concept_type stamped — D43). Reject → re-invoke, cap 2, then
               park+flag. A directly-parked create is NOT composed — km already wrote its
               digest body (D46). Unpark discards that digest body and recomposes.
               merge-slides keeps each predecessor as a `_vN` variant of the new slide; the
               fresh compose is its active variant (D47).
 5 FINISH      lead runs km validate; report (Backup Slides + why, flags, FIGURE NEEDED).
```
...
7. **Slide-variant cycling (D47):** `km get-variants` / `cycle-variant` + the enumeration
   filter + the merge-preserves-predecessors change (with tests), then the scaffold wiring
   (Vite `configureServer` endpoint + `setup/shortcuts.ts`). **Spike the reload-on-rename
   assumption first** (§design doc §9). **Mechanics only** — variant *creation* from lanes is
   deferred. Design: `docs/superpowers/specs/2026-07-22-slide-variant-cycling-design.md`; ADR-0004.
